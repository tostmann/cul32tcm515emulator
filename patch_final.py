...
# 1. Ensure ERP1_MIN_PREAMBLE is 3
content = re.sub(r'#define ERP1_MIN_PREAMBLE\s+6', '#define ERP1_MIN_PREAMBLE     3', content)

# 2. Fix radio_transmit function
new_tx = """void radio_transmit(const uint8_t *data, uint8_t len) {
...
"""
content = re.sub(r'void radio_transmit\(const uint8_t \*data, uint8_t len\) \{.*?(?=\nstatic inline int hex2int)', new_tx, content, flags=re.DOTALL)
