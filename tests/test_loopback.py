...
try:
    with serial.Serial(EMU_ID, 57600, timeout=2) as ser:
        # RPS Telegram
        data = bytes.fromhex("F600FFFFFFFF30")
        opt = bytes.fromhex("03FFFFFFFF0000")
        esp3_frame = build_esp3_packet(0x01, data, opt)
        
        print(f"Sende ERP1 Paket: {esp3_frame.hex()}")
